	...
	class AdaptiveImpedanceController: public controller_interface::Controller<hardware_interface::EffortJointInterface>
	{
		...
		bool init(hardware_interface::EffortJointInterface* hw, ros::NodeHandle &n)
  		{	
			...
			// urdf
			urdf::Model urdf;			
			if (!urdf.initParam("bionicarm/robot_description"))
			{
			...
			for (size_t i=0; i<n_joints_; i++)
			{
				...
				if ( n.getParam("/bionicarm/adaptive_impedance_controller/joint" + si + "/tdc/mbar", Mbar[i]) )
				{
					Mbar_(i) = Mbar[i];
				}
				else
				{
					std::cout << "/bionicarm/adaptive_impedance_controller/joint" + si + "/tdc/mbar" << std::endl;
					ROS_ERROR("Cannot find tdc/mbar gain");
					return false;
				}

				if ( n.getParam("/bionicarm/adaptive_impedance_controller/joint" + si + "/tdc/r", Ramda[i]) )
				{
					Ramda_(i) = Ramda[i];
				}
				else
				...
				if ( n.getParam("/bionicarm/adaptive_impedance_controller/joint" + si + "/tdc/a", Alpha[i]) )
				{
					Alpha_(i) = Alpha[i];
				}
				else
				...
				if ( n.getParam("/bionicarm/adaptive_impedance_controller/joint" + si + "/tdc/w", Omega[i]) )
				{
					Omega_(i) = Omega[i];
				}
				else
			...
			if (!n.getParam("/bionicarm/adaptive_impedance_controller/aic/fd", Fd_temp_))
			{
				ROS_ERROR("Cannot find aci/fd");
				return false;
			}

			if (!n.getParam("/bionicarm/adaptive_impedance_controller/aic/m", M_))
			{
				ROS_ERROR("Cannot find aci/m");
				return false;
			}

			if (!n.getParam("/bionicarm/adaptive_impedance_controller/aic/b", B_))
			{
				ROS_ERROR("Cannot find aci/b");
				return false;
			}

			if (!n.getParam("/bionicarm/adaptive_impedance_controller/mode", experiment_mode_))
			{
			...
			sub_forcetorque_sensor_ = n.subscribe<geometry_msgs::WrenchStamped>("/bionicarm/bionicarm/ft_sensor_topic", 1, &AdaptiveImpedanceController::updateFTsensor, this);
