  ...
  public:
    bool init(hardware_interface::EffortJointInterface *hw, ros::NodeHandle &n)
    {
        ...
        for (size_t i = 0; i < n_joints_; i++)
        {
            ...
            if (n.getParam("/bionicarm/computed_torque_controller_clik/gains/bionicarm_joint" + si + "/pid/p", Kp[i]))
            {
                Kp_(i) = Kp[i];
            }
            else
            {
                std::cout << "/bionicarm/computed_torque_controller_clik/gains/bionicarm_joint" + si + "/pid/p" << std::endl;
                ROS_ERROR("Cannot find pid/p gain");
                return false;
            }

            if (n.getParam("/bionicarm/computed_torque_controller_clik/gains/bionicarm_joint" + si + "/pid/i", Ki[i]))
            {
                Ki_(i) = Ki[i];
            }
            else
            ...
            if (n.getParam("/bionicarm/computed_torque_controller_clik/gains/bionicarm_joint" + si + "/pid/d", Kd[i]))
            {
                Kd_(i) = Kd[i];
            }
            else
        ...
        if (ctr_obj_ == 1)
        {
            if (!n.getParam("/bionicarm/computed_torque_controller_clik/clik_gain/K_regulation", K_regulation_))
            {
        ...
        {
            if (!n.getParam("/bionicarm/computed_torque_controller_clik/clik_gain/K_tracking", K_tracking_))
            {
